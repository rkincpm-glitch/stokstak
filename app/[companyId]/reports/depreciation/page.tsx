...
export default function DepreciationReport() {
  ...
  useEffect(() => {
    const run = async () => {
      ...
      const { data, error } = await supabase
        .from("items")
        .select("id, name, purchase_price, purchase_date, useful_life_years, depreciation_rate")
  ...
  const rows = useMemo(() => {
    ...
    return items.map((i) => {
      ...
      const life = i.useful_life_years ?? null;
      const ratePct = i.depreciation_rate ?? null;
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-t">
                ...
                <td className="px-4 py-3 text-right">{r.useful_life_years ?? "—"}</td>
                <td className="px-4 py-3 text-right">{r.depreciation_rate ?? "—"}</td>
